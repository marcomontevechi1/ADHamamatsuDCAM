...
class DCAMERR(IntEnum):
    ...
    INVALID_REALTIMEGAINCORRECTREGIONS = (
        -2147481533
    )  # 0x80000843, the combination of hpos and hsize for realtime correct region is invalid. e.g. DCAM_IDPROP_REALTIMECORRECTREGION_HPOS + DCAM_IDPROP_REALTIMECORRECTREGION_HSIZE is grater than the number of horizontal pixel.
    NOTSUPPORT = (
        -2147479805
    ...
    DELAYEDFRAME = (
        -2147479799
    )  # 0x80000f09, the frame waiting re-load from hardware buffer with SNAPSHOT(EX) of DEVICEBUFFER MODE
    FAILRELOADFRAME = (
        -2147479798
    )  # 0x80000f0a, failed to re-load frame from hardware buffer with SNAPSHOT(EX) of DEVICEBUFFER MODE
    CANCELRELOADFRAME = (
        -2147479797
    )  # 0x80000f0b, cancel to re-load frame from hardware buffer with SNAPSHOT(EX) of DEVICEBUFFER MODE
...
class DCAM_IDPROP(IntEnum):
    ...
    TRIGGERENABLE_BURSTTIMES = (
        1049664  # 0x00100440, R/W, mode,    "TRIGGER ENABLE BURST TIMES"
    )
    ...
    SHADINGCALIB_TARGETMIN = (
        3671680  # 0x00380680, R/W, long,  "SHADING CALIB TARGET MIN"
    )
    # TAP CALIBRATION
    ...
    MULTILINESENSOR_READOUTMODE = (
        4195200  # 0x00400380, R/W, mode, "MULTI LINE SENSOR READOUT MODE"
    )
    MULTILINESENSOR_TOP = 4195216  # 0x00400390, R/W, long, "MULTI LINE SENSOR TOP"
    MULTILINESENSOR_HEIGHT = (
        4195232  # 0x004003A0, R/W, long,  "MULTI LINE SENSOR HEIGHT"
    )
    # output mode
    ...
    BURIEDDATA_MODE = 4196864  # 0x00400A00, R/W, mode, "BURIED DATA MODE"
    # Group: ROI
    ...
    INTERNALLINERATE_CONTROL = (
        4208752  # 0x00403870, R/W, mode,    "INTERNAL LINE RATE CONTROL"
    )
    # system information
    ...
    IMAGEDETECTOR_PIXELWIDTH = (
        4327440  # 0x00420810, R/O, micro-meter, "IMAGE DETECTOR PIXEL WIDTH"
    )
    IMAGEDETECTOR_PIXELHEIGHT = (
        4327456  # 0x00420820, R/O, micro-meter, "IMAGE DETECTOR PIXEL HEIGHT"
    )
    IMAGEDETECTOR_PIXELNUMHORZ = (
        4327472  # 0x00420830, R/O, long,  "IMAGE DETECTOR PIXEL NUM HORZ"
    )
    IMAGEDETECTOR_PIXELNUMVERT = (
        4327488  # 0x00420840, R/O, long,  "IMAGE DETECTOR PIXEL NUM VERT"
    )
    # frame bundle
    ...
    NUMBEROF_IMAGEBLOCK = 5111808  # 0x004E0000, R/O, long, "NUMBER OF IMAGE BLOCK"
    IMAGEBLOCK_FIRSTBYTESOFFSET = 5115904  # 0x004E1000
    IMAGEBLOCK_ACTUALXPOS = 5120000  # 0x004E2000
    IMAGEBLOCK_ACTUALYPOS = 5124096  # 0x004E3000
    IMAGEBLOCK_ACTUALXSIZE = 5128192  # 0x004E4000
    IMAGEBLOCK_ACTUALYSIZE = 5132288  # 0x004E5000
    IMAGEBLOCK_OVERLAPLEFTSIZE = 5136384  # 0x004E6000
    ...
    PRIMARYBUFFER_TOTALBYTES = (
        16715824  # 0x00FF1030, R/W, long,   "PRIMARY BUFFER TOTALBYTES"
    )
    PRIMARYBUFFER_TOTALBYTES_MB = (
        16715840  # 0x00FF1040, R/W, long,    "PRIMARY BUFFER TOTALBYTES MB"
    )
...
class DCAMPROP_ATTR(Structure):
    ...
    @property
    def arraybase(self):
        return True if self.attribute2 & DCAM_PROP.ATTR2.ARRAYBASE else False

    @property
    def arrayelement(self):
        return True if self.attribute2 & DCAM_PROP.ATTR2.ARRAYELEMENT else False

    @property
    def type(self):
        return DCAM_PROP.TYPE(self.attribute & DCAM_PROP.TYPE.MASK)

    @property
    def hasrange(self):
        return bool(self.attribute & DCAM_PROP.ATTR.HASRANGE)

    @property
    def hasstep(self):
        return bool(self.attribute & DCAM_PROP.ATTR.HASSTEP)

    @property
    def hasdefault(self):
        return bool(self.attribute & DCAM_PROP.ATTR.HASDEFAULT)
